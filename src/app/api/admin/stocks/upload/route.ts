...
export async function POST(req: NextRequest) {
  try {
    ...
    else if (isCsv) rows = await parseCsv(buffer);
    else {
      return NextResponse.json({ message: '지원하지 않는 파일 형식입니다(.xlsx, .csv)' }, { status: 400 });
    }
    ...
    let inserted = 0;
    let skipped = 0;
    let skippedNoData = 0; // 필수 필드 누락
    let skippedExists = 0; // 이미 존재
    ...
    for (const raw of rows) {
      ...
      // 디버깅 로그 추가
      console.log(`Row ${total}:`, { raw, normalized: data });

      if (!data.isinCode || !data.shortName) {
        skipped += 1;
        skippedNoData += 1;
        console.log(`Skipped: 필수 필드 누락 - isinCode: ${data.isinCode}, shortName: ${data.shortName}`);
        continue;
      }
      ...
      if (exists) {
        skipped += 1;
        skippedExists += 1;
        console.log(`Skipped: 이미 존재 - isinCode: ${data.isinCode}`);
        continue;
      }
    ...
    const hint = err?.code === 'MODULE_NOT_FOUND' ? 'xlsx, csv-parse 패키지 설치가 필요합니다.' : undefined;
